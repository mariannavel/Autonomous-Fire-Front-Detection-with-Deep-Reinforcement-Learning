...
def train(epoch):
    ...
    for batch_idx, (inputs, targets) in tqdm.tqdm(enumerate(trainloader), total=len(trainloader)):
        ...
        inputs_map = torch.nn.functional.interpolate(inputs.clone(), (64, 64))  # downsample 256x256 images
        ...
        preds_sample = preds_sample + preds_lr # giati ta prosthetei? den einai to equation (7)?
...
def test(epoch):
    ...
    for batch_idx, (inputs, targets) in tqdm.tqdm(enumerate(testloader), total=len(testloader)):
        ...
        #     plt.imshow( inputs[i].permute(1, 2, 0) ) # .type("torch.ByteTensor")
        #     plt.show()
        if (args.model == 'R32_Landsat-8'):
            inputs = inputs.float().permute(0,3,1,2)
            targets = torch.Tensor([int(val) for val in targets])
        ...
        preds = preds_hr + preds_lr # equation (7)?? posa patches ginontai sample??
    ...
    torch.save(state, args.cv_dir+'/Landsat-8/F2Stream_ckpt_E_%d_A_%.3f_R_%.2E'%(epoch, accuracy, reward))
...
if args.ckpt_lr_cl is not None:
    ...
    #     utils.load_weights_to_flatresnet(checkpoint, rnet_lr) # it was rnet...?
    # else:
